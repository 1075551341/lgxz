 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-12 13:54:52
 * @Description: 一次调频--贡献电量补偿
-->
<script setup>
...
let tableData1 = reactive({
  column: [
    ...
    { label: '贡献电量合格率', field: 'avg', slotName: 'avg' },
    { label: '补偿分数', field: 'bcvalue', slotName: 'bcvalue' },
    { label: '实际贡献量', field: 'pvalue' },
    { label: '理论贡献量', field: 'llvalue' },
    { label: '机组扰动数量', field: 'iscount' },
    { label: '小扰动数量', field: 'mincount' },
    { label: '大扰动数量', field: 'maxcount' },
...
<template>
  <div class="contributionPowerCompensation">
    <div class="tops">
      <div class="topLeft">
        <el-select
          v-model="selectValue"
    ...
    <div id="topsContent">
      <div class="topsContent">
        <div class="topsBlow">
            ...
            <div class="topAvatar">
              ...
              <div class="under">
                <div class="underLine">
                  <p>
                    实际贡献电量<i>&nbsp;</i
                    ><span>{{ filterNumber(staticData.data?.pvalue) }}</span>
                  </p>
                  <p>
                    <i>&nbsp;</i>理论贡献电量<span>{{
                      filterNumber(staticData.data?.llvalue)
                    }}</span>
                  </p>
                </div>
              </div>
          ...
          <div class="charts">
            <div v-show="chartVisible" id="chartBar" class="chartItem"></div>
          </div>
        </div>
        <table-view
          class="tableView"
        ...
        >
          ...
          <template #bcvalue="scope">
            <div>
              {{ buttonMSg.type != 1 ? filterNumber(scope.row.bcvalue) : '--' }}
            </div>
          </template>
          ...
          >
            <div class="stateModal">
              <div>
                <p>理论/实际贡献电量：</p>
                <p class="text">
                  {{ filterNumber(modalLineChartData.row?.th) }} /
                  {{ filterNumber(modalLineChartData.row?.rea) }} (兆瓦/MW)
                </p>
              </div>
            </div>
            <div class="modals">
