 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-09 10:18:23
 * @Description: agc--可用率考核与补偿
-->
<script setup>
...
const queryDayStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAgcOperationalratedayEntity/queryStatistics', {
...
const queryMonthStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAgcOperationalratedayEntity/queryMonthStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAgcOperationalratemonthlyEntity/queryStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
let tableData1 = reactive({
  column: [
    ...
    { label: '投运率', field: 'tyl', slotName: 'tyl' },
...
// 获取表格1 日/月数据
const getRequestFn = () => {
  axiosRequest('/glbecoPhyunitAgcOperationalratedayEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
const getYearRequestFn = () => {
  axiosRequest('/glbecoPhyunitAgcOperationalratemonthlyEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
    size: tableAndPaginationConfig1.value.pagesizeConfig.size,
    current: tableAndPaginationConfig1.value.pagesizeConfig.current
...
let tableData2 = reactive({
  column: [
    ...
    { label: '投退标识(投入/退出)', field: 'agcstatus', slotName: 'agcstatus' },
    { label: '投退状态', field: 'agcstatusText', slotName: 'agcstatusText' }
  ],
  showIndexColumn: true,
  data: []
})
...
const getQueryPageRequestFn = () => {
  axiosRequest('/api/glbecoPHPUnitAgcEntity/queryAgcOperationalratePage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
<template>
  <div class="agcAvailableRateExam">
    <div class="tops">
      <div class="topLeft">
        <el-select
          v-model="selectValue"
    ...
    <div id="topsContent">
      <div class="topsContent">
          ...
          <div class="topAvatar">
            ...
            <div class="under">
              <div class="underLine">
                <p>
                  同比<i>&nbsp;</i
                  ><span>{{ filterNumber(staticData.data?.khvalueTb, '%') || 0 }}</span>
                </p>
                <p>
                  <i>&nbsp;</i>环比<span>{{
                    filterNumber(staticData.data?.khvalueHb, '%') || 0
                  }}</span>
                </p>
              </div>
            </div>
          ...
          <div class="topAvatar">
            ...
            <div class="under">
              <div class="underLine">
                <p>
                  同比<i>&nbsp;</i
                  ><span>{{ filterNumber(staticData.data?.bcvalueTb, '%') || 0 }}</span>
                </p>
                <p>
                  <i>&nbsp;</i>环比<span>{{
                    filterNumber(staticData.data?.bcvalueHb, '%') || 0
                  }}</span>
                </p>
              </div>
            </div>
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                <p>平均投运率</p>
                <p>
                  <span>{{ filterNumber(staticData.data?.tyl) || 0 }} </span> %
                </p>
              </div>
            </div>
            <div class="under">
              <div class="underLine">
                <p>
                  投入时长<span>{{ filterNumber(staticData.data?.starttimevalue, '%') || 0 }}</span>
                </p>
                <p>
                  <i>&nbsp;</i>退出时长<span>{{
                    filterNumber(staticData.data?.stoptimevalue, '%') || 0
                  }}</span>
                </p>
              </div>
            </div>
        ...
        >
          <template #tyl="scope">
            <div>{{ filterNumber(scope.row.tyl, '%') }}</div>
          </template>
        ...
        >
          <template #agcstatus="scope">
            <div :class="[scope.row.agcstatus != 1 && 'colorRed']">
              {{ scope.row.agcstatus || '--' }}
            </div>
          </template>
          <template #agcstatusText="scope">
            <div :class="[scope.row.agcstatus != 1 && 'colorRed']">
              {{
                scope.row.agcstatus == 1 ? '投入' : scope.row.agcstatus == 0 ? '退出' : '状态异常'
              }}
            </div>
          </template>
        </table-view>
...
.agcAvailableRateExam {
  ...
  :deep(.el-select .el-input.is-focus .el-input__wrapper) {
    box-shadow: 0 0 0 1px var(--colorRed) inset;
  }
  .tops {
    display: flex;
