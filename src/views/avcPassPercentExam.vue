 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-05 16:45:43
 * @Description: avc--合格率考核
-->
<script setup>
...
const queryDayStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAvcIsupDayEntity/queryStatistics', {
...
const queryMonthStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAvcIsupDayEntity/queryMonthStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAvcIsupMonthlyEntity/queryStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
let lineChartData = reactive({
  value: { dataTime: [], avcins: [], vgen: [] }
})
const getLineChartFn = () => {
  axiosRequest('/glbecoPhyunitAvcEntity/queryCurve', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const drawLine = () => {
  ...
  option = {
    ...
    graphic: {
      ...
      top: 'middle',
      silent: true, // 不响应事件
      invisible: lineChartData.value.avcins.length || lineChartData.value.vgen.length > 0, // 有数据就隐藏
      style: {
        fill: '#909399',
        fontWeight: 'normal',
        text: '暂无数据',
    ...
    series: [
      {
        ...
        data: lineChartData.value.avcins || []
      },
      {
        name: '母线电压',
        // position: [10, 10],
...
const queryMonthChartFn = () => {
  axiosRequest('/glbecoPhyunitAvcIsupDayEntity/queryPassRateBarChart', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearBarChartFn = () => {
  axiosRequest('/glbecoPhyunitAvcIsupMonthlyEntity/queryPassRateBarChart', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
let tableData1 = reactive({
  column: [
    ...
    { label: '平均合格率', field: 'isavg', slotName: 'isavg' },
...
// 获取表格1 日/月数据
const getRequestFn = () => {
  axiosRequest('/glbecoPhyunitAvcIsupDayEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
const getYearRequestFn = () => {
  axiosRequest('/glbecoPhyunitAvcIsupMonthlyEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
    size: tableAndPaginationConfig1.value.pagesizeConfig.size,
    current: tableAndPaginationConfig1.value.pagesizeConfig.current
  ...
  <div class="avcPassPercentExam">
    ...
    <div id="topsContent">
      <div class="topsContent">
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                ...
                <p>
                  <span
                    >{{ staticData.data?.sumassess || (buttonMSg.type == 1 ? '-' : 0) }} /{{
                      staticData.data?.avgyield || 0
                    }}
                  </span>
                  %
                </p>
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                <p>总点数</p>
                <p>
                  <span>{{ staticData.data?.sumcount || 0 }} </span> 个
                </p>
              </div>
            </div>
            <div class="under">
              <div class="underLine">
                <p>
                  不合格<i>&nbsp;</i><span>{{ staticData.data?.sumassessnocount || 0 }}</span>
                </p>
                <p>
        ...
        >
          <template #isavg="scope">
            <div>{{ filterNumber(scope.row.isavg, '%') }}</div>
          </template>
        ...
        >
          ...
          <template #cause="scope">
            <div :class="[scope.row.stat != 1 && 'colorRed']">{{ scope.row.cause || '--' }}</div>
          </template>
