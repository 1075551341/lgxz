 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-05 15:09:48
 * @Description:  agc-- 响应时间考核(新能源)
-->
<script setup>
...
const queryDayStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyratetimeDayEntity/queryStatistics', {
...
const queryMonthStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyratetimeDayEntity/queryMonthStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyratetimeEntity/queryStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryMonthChartFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyratetimeDayEntity/queryPassRateBarChart', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
let tableData1 = reactive({
  column: [
    ...
    { label: '总点数', field: 'count' },
    { label: '不合格点数', field: 'nocount', slotName: 'nocount' },
...
// 获取表格1 日/月数据
const getRequestFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyratetimeDayEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
const getYearRequestFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyratetimeEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
    size: tableAndPaginationConfig1.value.pagesizeConfig.size,
    current: tableAndPaginationConfig1.value.pagesizeConfig.current
...
let tableData2 = reactive({
  column: [
    ...
    { label: '是否合格(合格/不合格)', field: 'isres', slotName: 'isres' },
...
<template>
  <div class="agcResponseTimeExamNewEnergy">
    <div class="tops">
      <div class="topLeft">
        <el-select
          v-model="selectValue"
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                ...
                <p>
                  <span
                    >{{ staticData.data?.khvalue || (buttonMSg.type == 1 ? '-' : 0) }}/{{
                      staticData.data?.avgisup || 0
                    }}</span
                  >
                  %
                </p>
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                <p>总点数</p>
                <p>
                  <span>{{ staticData.data?.count || 0 }}</span
                  >个
                </p>
              </div>
            </div>
            <div class="under">
              <div class="underLine">
                <p>
                  不合格<i>&nbsp;</i><span>{{ staticData.data?.nocount || 0 }}</span>
                </p>
                <p>
                  合格<i>&nbsp;</i><span>{{ staticData.data?.sumassesscount || 0 }}</span>
                </p>
              </div>
            </div>
          </div>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="td">响应时间</td>
                      <td class="td">
                        <p v-show="modalLineChartData.row.restime">
                          {{ filterNumber(modalLineChartData.row?.restime, '秒') }}
                        </p>
                      </td>
                    </tr>
                    <tr>
