 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-12 13:52:35
 * @Description: agc--贡献电量合格率考核(新能源)
-->
<script setup>
...
const queryDayStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyrateDayEntity/queryStatistics', {
...
const queryMonthStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyrateDayEntity/queryMonthStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyrateEntity/queryStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
console.log('new echarts::: ', new echarts.graphic.LinearGradient())
// 柱状图
const drawBar = () => {
...
let tableData1 = reactive({
  column: [
    ...
    { label: '贡献电量合格率', field: 'gxdlrage', slotName: 'gxdlrage' },
...
// 获取表格1 日/月数据
const getRequestFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyrateDayEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
const getYearRequestFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyrateEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
    size: tableAndPaginationConfig1.value.pagesizeConfig.size,
    current: tableAndPaginationConfig1.value.pagesizeConfig.current
...
let tableData2 = reactive({
  column: [
    ...
    { label: '是否合格', field: 'isres', slotName: 'isres' },
...
<template>
  <div class="agcPassPercentExamNewEnergy">
    <div class="tops">
      <div class="topLeft">
        <el-select
          v-model="selectValue"
    ...
    <div id="topsContent">
      <div class="topsContent">
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                <p>考核分数</p>
                <p>
                  <span>{{ staticData.data?.sumassess || (buttonMSg.type == 1 ? '-' : 0) }}</span>
                  分
                </p>
              </div>
            </div>
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                <p>贡献电量合格率</p>
                <p>
                  <span>{{ staticData.data?.gxdlrage || 0 }}</span> %
                </p>
              </div>
            </div>
            <div class="under">
              <div class="underLine">
                <p>
                  理论贡献电量<i>&nbsp;</i><span>{{ staticData.data?.llgxdlvalue || 0 }}</span>
                </p>
                <p>
                  实际贡献电量<i>&nbsp;</i><span>{{ staticData.data?.sjgxdlvalue || 0 }}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <template #gxdlrage="scope">
            <div>{{ filterNumber(scope.row.gxdlrage, '%') }}</div>
          </template>
              ...
              <div class="modalTable">
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="td">实际贡献电量</td>
                      <td class="td">
                        {{ modalLineChartData.row?.sjgxdlvalue }}
                      </td>
                    </tr>
                    <tr>
                      <td class="td">理论贡献电量</td>
                      <td class="td">{{ modalLineChartData.row?.llgxdlvalue }}</td>
                    </tr>
                    <tr>
                      <td class="td">最大有功</td>
                    ...
                    <tr>
                      ...
                      <td class="td">
                        {{
                          modalLineChartData.row?.isxd == 1
                            ? '是'
                            : modalLineChartData.row?.isxd == 0
                              ? '否'
                              : '状态异常'
                        }}
                      </td>
