 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-16 15:07:38
 * @Description:  agc-- 响应时间考核(常规)
-->
<script setup>
...
const queryDayStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAgcResponsetimedayEntity/queryStatistics', {
...
const queryMonthStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAgcResponsetimedayEntity/queryMonthStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitAgcResponsetimemonthlyEntity/queryStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const getLineChartFn = () => {
  axiosRequest('/api/glbecoPHPUnitAgcEntity/queryAgcAndPRealCurve', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
    ...
    xAxis: {
      ...
      axisLine: {
        ...
        lineStyle: {
          ...
          shadowColor: '#999' //设置阴影的颜色 // 坐标轴线线的颜色
        }
      }
    },
    yAxis: {
...
const queryMonthChartFn = () => {
  axiosRequest('/glbecoPhyunitAgcResponsetimedayEntity/queryPassRateBarChart', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearBarChartFn = () => {
  axiosRequest('/glbecoPhyunitMonthlyratetimeEntity/queryPassRateBarChart', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
let tableData1 = reactive({
  column: [
    ...
    { label: '平均合格率', field: 'avgisup', slotName: 'avgisup' },
    { label: '考核分数', field: 'khvalue', slotName: 'khvalue' },
    { label: '总指令次数', field: 'cmdCount' },
    // { label: '不合格次数', field: 'nocount', slotName: 'nocount' },
    { label: '平均响应时间', field: 'avgisupsl', slotName: 'avgisupsl' },
...
// 获取表格1 日/月数据
const getRequestFn = () => {
  axiosRequest('/glbecoPhyunitAgcResponsetimedayEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
const getYearRequestFn = () => {
  axiosRequest('/glbecoPhyunitAgcResponsetimemonthlyEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
    size: tableAndPaginationConfig1.value.pagesizeConfig.size,
    current: tableAndPaginationConfig1.value.pagesizeConfig.current
...
let tableData2 = reactive({
  column: [
    ...
    { label: '响应时刻', field: 'ressttime' },
    { label: '响应时刻有功', field: 'presst' },
    {
      label: '指令差/装机占比',
...
<template>
  <div class="agcResponseTimeExamCommonRule">
    <div class="tops">
      <div class="topLeft">
        <el-select
          v-model="selectValue"
    ...
    <div id="topsContent">
      <div class="topsContent">
              ...
              <div>
                ...
                <p>
                  <span
                    ...
                    }}/{{ filterNumber(staticData.data?.avgisupsl) || 0 }}</span
                  >
                </p>
              </div>
            </div>
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                <p>AGC指令次数</p>
                <p>
                  <span>{{ filterNumber(staticData.data?.cmdCount) || 0 }}</span
                  >个
                </p>
              </div>
            </div>
            <div class="under">
              <div class="underLine">
                <p>
                  不合格<i>&nbsp;</i><span>{{ filterNumber(staticData.data?.nocount) || 0 }}</span>
                </p>
                <p>
                  <i>&nbsp;</i>合格<span>{{
                    filterNumber(staticData.data?.sumassesscount) || 0
                  }}</span>
                </p>
              </div>
            </div>
        ...
        >
          <template #avgisup="scope">
            <div>{{ filterNumber(scope.row.avgisup, '%') }}</div>
          </template>
          <!-- <template #khvalue="scope">
            <div>
              {{ scope.row.khvalue ? filterNumber(scope.row.khvalue) : '--' }}
            </div>
          </template> -->
          ...
          <template #avgisupsl="scope">
            <div>{{ filterNumber(scope.row.avgisupsl, '秒') }}</div>
          </template>
          ...
          >
            ...
            <div class="modals">
              ...
              <div class="modalTable">
                ...
                ><span v-show="tooltipData2.val?.seriesName">{{
                  filterNumber(tooltipData2.val?.value)
                }}</span>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="td">
                        {{ modalLineChartData.row?.pmintime || '--' }}
                      </td>
                    </tr>
                  </tbody>
                </table>
