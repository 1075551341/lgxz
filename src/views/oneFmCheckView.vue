 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-10 16:05:16
 * @Description: 一次调频考核
-->
<script setup>
...
const changeListFn = (id, val) => {
  if (val) {
    ...
    hostMsg.value = val?.maxCap //默认装机容量
  }
}
// 监听--机组切换
...
let tableData1 = reactive({
  column: [
    ...
    { label: '总考核分数', field: 'value' },
    { label: '小扰动考核分数', field: 'khvalue' },
    { label: '小扰动平均合格率', field: 'avg', slotName: 'avg' },
    { label: '大频差扰动考核分数', field: 'islargevalue' },
    { label: '大频差不合格次数', field: 'islargecount', slotName: 'islargecount' },
    { label: '大频差扰动数量', field: 'maxcount' },
    { label: '过调节考核分数', field: 'gtjkhvalue' },
    { label: '过调节数量', field: 'gtjcount' },
...
let tableData2 = reactive({
  column: [
    ...
    { label: '是否大扰动', field: 'islarge', slotName: 'islarge' },
    ...
    { label: '实际贡献电量', field: 'pvalue' },
    { label: '理论贡献电量', field: 'llvalue' },
  ...
  <div class="oneFmCheck">
    ...
    <div id="topsContent">
      <div class="topsContent">
            ...
            <div class="topAvatar">
              <div class="blow">
                ...
                <div>
                  ...
                  <p>
                    <span>{{
                      filterNumber(staticData.data?.value) || (buttonMSg.type == 1 ? '-' : 0)
                    }}</span>
                    分
                  </p>
                </div>
              ...
              <div class="under">
                <div class="underLine">
                  <p>&nbsp;</p>
                  <!-- <p>
                    小扰动<i>&nbsp;</i><span>{{ staticData.data?.mincount || 0 }}</span>
                  </p>
                  <p>
                    大扰动<i>&nbsp;</i><span>{{ staticData.data?.maxcount || 0 }}</span>
                  </p> -->
                </div>
              </div>
            </div>
            <div class="topAvatar">
              <div class="blow">
                ...
                <div>
                  <p>小扰动平均合格率</p>
                  <p>
                    <span>{{ filterNumber(staticData.data?.avg) }} </span> %
                  </p>
                </div>
              </div>
              <div class="under">
                <div class="underLine">
                  <p>&nbsp;</p>
                  <!-- <p>
                    实际<i>&nbsp;</i><span>{{ staticData?.pvalue || 0 }}</span>
                  </p>
                  <p>
                    理论<i>&nbsp;</i><span>{{ staticData.data?.llvalue || 0 }}</span>
                  </p> -->
                </div>
              </div>
            </div>
            <div class="topAvatar">
              <div class="blow">
                ...
                <div>
                  <p>机组扰动数量</p>
                  <p>
                    <span>{{ filterNumber(staticData.data?.iscount) }}</span> 个
                  </p>
                </div>
              </div>
              <div class="under">
            ...
            <div class="topAvatar">
              <div class="blow">
                ...
                <div>
                  <p>过调节考核分数</p>
                  <p>
                    <span>{{ filterNumber(staticData.data?.gtjkhvalue) }}</span> 分
                  </p>
                </div>
              </div>
              <div class="under">
        ...
        >
          <template #avg="scope">
            <div>{{ filterNumber(scope.row.avg, '%') }}</div>
          </template>
        ...
        >
          <template #islarge="scope">
            <div>
              {{ scope.row.islarge == 1 ? '是' : scope.row.islarge == 0 ? '否' : '状态异常' }}
            </div>
          </template>
          <template #stat="scope">
        ...
        <div class="tableDialog">
          ...
          >
            <div class="stateModal">
              <div>
                <p>扰动类型：</p>
                <p class="text" v-if="modalLineChartData.row.islarge == 1">大扰动</p>
                <p class="text" v-else-if="modalLineChartData.row.islarge == 0">小扰动</p>
                <p class="text" v-else>扰动类型异常</p>
              </div>
              <div>
                <p>状态：</p>
