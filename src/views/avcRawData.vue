 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-09 10:26:36
 * @Description: AVC考核与补偿--avc原始数据
-->
<script setup>
...
let drawLine = () => {
  ...
  option = {
    ...
    graphic: {
      ...
      top: 'middle',
      silent: true, // 不响应事件
      invisible: lineChartData.value.avcins.length > 0, // 有数据就隐藏
      style: {
        fill: '#909399',
        fontWeight: 'normal',
        text: '暂无数据',
...
let tableData1 = reactive({
  column: [
    ...
    { label: '偏差/占比', field: 'pc', slotName: 'pc' },
    { label: 'AGC投退标识', field: 'avcstatus', slotName: 'avcstatus' },
    { label: 'AGC投退状态', slotName: 'avcstatusText' }
  ],
  data: []
})
...
// 获取表格数据
const getRequestFn = () => {
  axiosRequest('/glbecoPhyunitAvcEntity/queryOriginalPage', {
    phyUnitId: selectListId.value,
    beginDate: startTime.value,
    endDate: endTime.value,
        ...
        <div id="tables">
          ...
          >
            ...
            <template #avcstatusText="scope">
              <div :class="[scope.row.avcstatus != 1 && 'colorRed']">
                {{
                  scope.row.avcstatus == 1 ? '投入' : scope.row.avcstatus == 0 ? '退出' : '状态异常'
                }}
              </div>
            </template>
          </table-view>
