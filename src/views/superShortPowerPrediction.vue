 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-16 15:04:07
 * @Description: 功率预测考核--超短期功率预测
-->
<script setup>
...
const queryDayStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycDayEntity/queryStatistics', {
...
const queryMonthStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycDayEntity/queryMonthStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearStatisticsFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycDayMonthlyEntity/queryStatistics', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const getLineChartFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycRnEntity/queryCurve', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryMonthChartFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycDayEntity/queryPassRateBarChart', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
const queryYearBarChartFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycDayMonthlyEntity/queryPassRateBarChart', {
    phyUnitId: selectListId.value,
    date: pickerDate.value
  }).then((res) => {
    if (res && res.code == 200) {
...
let tableData1 = reactive({
  column: [
    ...
    { label: '第一小时调和平均数(80%)', field: 'v4' },
    { label: '考核分数', field: 'v4kh' },
    { label: '第二小时(75%)', field: 'v8' },
    { label: '考核分数', field: 'v8kh' },
    { label: '第三小时(70%)', field: 'v12' },
    { label: '考核分数', field: 'v12kh' },
    { label: '第四小时(65%)', field: 'v16' },
    { label: '考核分数', field: 'v16kh' },
...
// 获取表格1 日/月数据
const getRequestFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycDayEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
const getYearRequestFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycDayMonthlyEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
    size: tableAndPaginationConfig1.value.pagesizeConfig.size,
    current: tableAndPaginationConfig1.value.pagesizeConfig.current
...
let tableData2 = reactive({
  column: [
    ...
    { label: '实际有功', field: 'p' },
    { label: '第一小时(V4)预测', field: 'V4' },
    { label: '误差值', field: 'DeviationvalueV4' },
    { label: '第二小时(V8)预测', field: 'V8' },
    { label: '误差值', field: 'DeviationvalueV8' },
    { label: '第三小时(V12)预测', field: 'V12' },
    { label: '误差值', field: 'DeviationvalueV12' },
    { label: '第四小时(V16)预测', field: 'V16' },
    { label: '误差值', field: 'DeviationvalueV16' }
  ],
  data: []
})
...
const getQueryPageRequestFn = () => {
  axiosRequest('/glbecoPhyunitCdqglycRnEntity/queryPage', {
    phyUnitId: selectListId.value,
    date: pickerDate.value,
...
<template>
  <div class="superShortPowerPrediction">
    <div class="tops">
      <div class="topLeft">
        <el-select
          v-model="selectValue"
      ...
      <div class="topsContent">
          ...
          <div class="topAvatar">
            <div class="blow">
              ...
              <div>
                <p>调和平均数(日)</p>
                <p>
                  <span>{{ staticData.data?.sumassess || 0 }} </span>万千瓦时
                </p>
              </div>
            </div>
            <div class="under">
              <div class="underLine">
                <p>
                  同比<i>&nbsp;</i><span>{{ staticData.data?.mincount || 0 }}</span>
                </p>
                <p>
                  <i>&nbsp;</i>环比<span>{{ staticData.data?.sumassess || 0 }}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <template #operate1="scope">
            <div @click="handelItem1(scope.$index, scope.row)"  v-debounce class="colorRed">查看</div>
          </template>
        </table-view>
