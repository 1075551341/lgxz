 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-16 16:30:29
 * @Description: 嵌套路由--首页(常规)
-->
<script setup>
...
const queryStatisticsFn = () => {
  axiosRequest('/homeEntity/queryStatistics', {
...
const getLineChartFn = () => {
  axiosRequest('/homeEntity/queryAgcAndPImplCurve', {
    phyUnitId: selectListId.value
  }).then((res) => {
    if (res && res.code == 200) {
      lineChartData.value = {
...
// 实时曲线折线图
const drawLine = () => {
  let chartDom = document.getElementById('lineChart1')
  let myChart = null
...
let perturbedLineChartData = reactive({
  value: { dataTime: [], power: [], theoryPower: [], frequency: [] }
})
const getPerturbedFn = () => {
  axiosRequest('/homeEntity/queryImplementCurve', {
    phyUnitId: selectListId.value
  }).then((res) => {
    if (res && res.code == 200) {
      perturbedLineChartData.value = {
        ...
        theoryPower: res.data.theoryPower,
    ...
    series: [
      {
        ...
        markLine: {
          ...
          data: [
            { xAxis: perturbedLineChartData.value.dataTime.length > 15 && 15 },
            {
              xAxis:
                perturbedLineChartData.value.dataTime.length > 30 &&
                perturbedLineChartData.value.dataTime.length - 15
            }
          ]
        },
        yAxisIndex: 0, //指定需要使用的Y轴
        // data: [30.7817, 30.8101, 30.8669, 30.8669]
        data: perturbedLineChartData.value.power
      },
      {
        name: '理论有功曲线',
        // position: [10, 10],
        ...
        yAxisIndex: 0, //指定需要使用的Y轴
        // data: [20, 30, 13, 11, 12, 42, 29],
        data: perturbedLineChartData.value.theoryPower
      },
      {
        name: '频率曲线',
        // position: [10, 10],
        ...
        yAxisIndex: 1, //指定需要使用的Y轴
        // data: [20, 30, 13, 11, 12, 42, 29],
        data: perturbedLineChartData.value.frequency
      }
    ]
  }
  // 绘制图表
      ...
      <div class="topAvatar">
        <div class="blow">
          ...
          <div>
            <p>日累计考核分数</p>
            <p>
              <span>{{ filterNumber(staticData.data?.khScore) }}</span> 分
            </p>
          </div>
        </div>
        <div class="under">
      ...
      <div class="topAvatar">
        <div class="blow">
          ...
          <div>
            <p>日累计补偿分数</p>
            <p>
              <span>{{ filterNumber(staticData.data?.bcScore) }}</span> 分
            </p>
          </div>
        </div>
        <div class="under">
    ...
    <div id="topsContent">
      <div class="topsContent">
        ...
        <!-- <div class="tablesContent">
          <div class="tableFit">
            <div class="agcDanger">
              <p class="agcDangerText">AGC性能告警</p>
            </div>
            <table-view
          ...
          <div class="tableFit">
            <div class="agcDanger">
              <p class="agcDangerText">一次调频性能告警</p>
            </div>
            <table-view
