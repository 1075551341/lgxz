 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-09 10:34:32
 * @Description: 一次调频原始数据
-->
<script setup>
...
const getLineChartRequestFn = () => {
  axiosRequest('glbecoPHPUnitFrequencyEntity/queryCurve', {
    ...
    if (res && res.code == 200) {
      ...
      // console.log('getLineChartRequestFn::: ', res, lineChartData)
    }
  })
}
  ...
  option = {
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        return `
        <div style="margin:10px 10px;width:180px;">
    ...
    graphic: {
      ...
      top: 'middle',
      silent: true, // 不响应事件
      invisible: lineChartData.value.p.length > 0, // 有数据就隐藏
      style: {
        fill: '#909399',
        fontWeight: 'normal',
        text: '暂无数据',
...
let tableData1 = reactive({
  column: [
    ...
    { label: '母线频率', field: 'f' },
    { label: '机端频率', field: 'funit' },
    { label: '机组转速', field: 'speed' },
    { label: '有功功率', field: 'p' }
  ],
  data: []
})
...
// 获取表格数据
const getRequestFn = () => {
  axiosRequest('/glbecoPHPUnitFrequencyEntity/queryPage', {
    phyUnitId: selectListId.value,
    beginDate: startTime.value,
    endDate: endTime.value,
