 ...
 * @LastEditors: wjs
 * @LastEditTime: 2024-01-16 10:08:06
 * @Description: AGC--原始数据
-->
<script setup>
...
const calendar = (date) => {
  ...
  // console.log('date::: ', date, minDate, maxDate)
  if (minDate && !maxDate) {
...
let drawLine = () => {
  ...
  option = {
    ...
    graphic: {
      ...
      top: 'middle',
      silent: true, // 不响应事件
      invisible: lineChartData.value?.agcins.length > 0, // 有数据就隐藏
      style: {
        fill: '#909399',
        fontWeight: 'normal',
        text: '暂无数据',
    ...
    series: [
      {
        ...
        markLine: {
          ...
          data: [
            { xAxis: lineChartData.value.dataTime.length > 15 && 15 },
            {
              xAxis:
                lineChartData.value.dataTime.length > 30 && lineChartData.value.dataTime.length - 15
            }
          ]
        },
...
let tableData1 = reactive({
  column: [
    ...
    { label: '有功功率', field: 'p' },
    { label: 'AGC投退标识', field: 'agcstatus', slotName: 'agcstatus' },
    { label: 'AGC投退状态', field: 'agcstatus', slotName: 'agcstatusText' }
  ],
  data: []
})
  ...
  <div class="agcRawData">
    ...
    <div id="topsContent">
      <div class="topsContent">
        <div class="charts">
          <div id="chart" class="chartItem" ref="chartItem"></div>
        </div>
        <div id="tables">
          <table-view
